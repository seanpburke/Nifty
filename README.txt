...
later. The _create function returns a "handle" for the queue, and even
...
The nft_queue_add call returns an error code. Most Nifty APIs that return
an error code, use zero to indicate success, and use POSIX error codes
like EINVAL or ENOMEM to indicate failure. These error codes are defined
in errno.h, but Nifty calls do not set the errno variable (though errno
may be set by system libraries that Nifty calls). The error codes that
a particular call can return, are defined in the package header file.
